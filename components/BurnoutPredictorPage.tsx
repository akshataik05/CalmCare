...
// Isolated Results Component for Safety
interface BurnoutResultViewProps {
    result: BurnoutPredictionResult;
    onReset: () => void;
    t: (key: any) => string;
}

const BurnoutResultView: React.FC<BurnoutResultViewProps> = ({ result, onReset, t }) => {
    const riskPercentage = result?.riskPercentage ?? 50;
    const factors = result?.factors || [];
    
    const riskColor = riskPercentage > 70 ? 'text-red-400' : riskPercentage > 35 ? 'text-amber-400' : 'text-emerald-400';
    const sortedFactors = [...factors]
        .filter(f => f && typeof f.contribution === 'number')
        ...
        if (active && payload && payload.length) {
            ...
            return (
                <div className="glass-card p-3 rounded-lg shadow-lg border border-border">
                    <p className="text-sm font-bold text-foreground">{data?.name || 'Unknown'}</p>
    ...
    // Find the factor with the highest contribution safely
    const topFactor = factors.length > 0 
        ? factors.reduce((max, factor) => 
            (factor?.contribution ?? 0) > (max?.contribution ?? 0) ? factor : max, factors[0]
          )
        : { name: 'Unknown', contribution: 0 };

    let recommendationText = '';
    const factorName = topFactor?.name || 'Unknown';
    switch (factorName) {
        case 'High Workload':
        ...
        case 'Poor Recovery':
            ...
            break;
        default:
            recommendationText = t('burnout_recommendation_workload') || 'Consider prioritizing tasks and scheduling brief breaks during shifts.';
            break;
    }
    ...
    return (
        <div className="animate-fadeIn w-full max-w-7xl mx-auto space-y-8 p-4">
            <div className="flex justify-between items-center">
                ...
                <button onClick={onReset} className="flex items-center gap-2 text-md sm:text-lg font-semibold text-muted-foreground hover:text-foreground">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                <div className="lg:col-span-3 glass-card p-8 sm:p-12 rounded-2xl flex flex-col">
                    <div className="text-center mb-8">
                        <p className={`text-8xl sm:text-[10rem] font-bold ${riskColor}`}>{riskPercentage}%</p>
                    </div>
                    <div>
...
const BurnoutPredictorPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!predictionResult || predictionResult.riskPercentage === -1) {
                setError(t('dashboard_predictor_error') || 'Unable to predict burnout risk.');
            } else {
                // Ensure factors exist, are not empty, and have names before sorting
                if (predictionResult.factors && predictionResult.factors.length > 0 && predictionResult.factors.every(f => f && f.name)) {
                     setResult(predictionResult);
                     setView('result');
                } else {
                    // Handle case where factors are missing or malformed
                    const fallbackResult = {
                        riskPercentage: predictionResult.riskPercentage ?? 50,
                        factors: [
            ...
            console.error(err);
            setError(t('dashboard_predictor_error') || 'An error occurred during prediction.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    if (view === 'result' && result) {
        return <BurnoutResultView result={result} onReset={handleReset} t={t} />;
    }
